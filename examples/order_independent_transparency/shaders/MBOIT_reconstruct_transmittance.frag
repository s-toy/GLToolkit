...
uniform sampler2D	uMomentB0Map;
uniform sampler2D	uMomentsMap1;
uniform sampler2D	uMomentsMap2;
...
void main()
{
	...
	float b_0 = texelFetch(uMomentB0Map, ivec2(gl_FragCoord.xy), 0).x;
	...
	vec4 b_1234 = texelFetch(uMomentsMap1, ivec2(gl_FragCoord.xy), 0).xyzw;
...
#elif NUM_MOMENTS == 6
	vec4 b_1234 = texelFetch(uMomentsMap1, ivec2(gl_FragCoord.xy), 0).xyzw;
	vec2 b_56 = texelFetch(uMomentsMap2, ivec2(gl_FragCoord.xy), 0).xy;
...
#elif NUM_MOMENTS == 8
	vec4 b_1234 = texelFetch(uMomentsMap1, ivec2(gl_FragCoord.xy), 0).xyzw;
	vec4 b_5678 = texelFetch(uMomentsMap2, ivec2(gl_FragCoord.xy), 0).xyzw;
