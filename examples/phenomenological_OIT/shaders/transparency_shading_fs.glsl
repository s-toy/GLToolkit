...
layout(location = 0) out vec4 _outAccumulatedReflectionColor;
layout(location = 1) out vec3 _outAccumulatedTransmission;
...
void main()
{
	...
	vec3  Lr = computeReflectColor();
	float t_average = (uTransmittance.r + uTransmittance.g + uTransmittance.b) / 3.0;
	float weight = pow(10.0 * (1.0 - 0.99 * gl_FragCoord.z) * uCoverage * (1.0 - t_average), 3.0);
	weight = clamp(weight, 0.01, 30.0);

	_outAccumulatedReflectionColor.rgb = weight * uCoverage * Lr;
	_outAccumulatedReflectionColor.a = weight * uCoverage * (1.0 - t_average);
	_outAccumulatedTransmission = 1.0 - uCoverage + uCoverage * uTransmittance;
}